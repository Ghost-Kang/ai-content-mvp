...
## 8. 单位经济护栏（D34 重算 2026-04-26 — token-based billing 实测）

> **D24 假设作废**：原 ¥6/60s clip 是按"clip-as-unit"估的，**实际 Volcengine Ark 按
> `usage.completion_tokens` 计费**，单价 ¥15 / 百万 tokens（doubao-seedance-1-0-pro-250528）。
> 实测（2026-04-26）：5s @ 480p = 49,005 tokens (¥0.74)，5s @ 720p = 103,818 tokens (¥1.56)。
> 60s 完整 workflow（17 帧 × ~3.5s）≈ **480p ¥9 / 720p ¥19**。

### 默认 480p 档（D33 锁定）

| 用户档位 | 视频条数/月 | Seedance 成本 | LLM 成本 | 数据源摊销 | 总成本 | 毛利 | 毛利率 |
|---|---|---|---|---|---|---|---|
| 轻度 | 10 | ¥90 | ¥10 | ¥30 | ¥130 | ¥870 | 87% |
| 中度 | 30 | ¥270 | ¥30 | ¥30 | ¥330 | ¥670 | 67% |
| 重度（D23 上限）| 60 | ¥540 | ¥60 | ¥30 | ¥630 | ¥370 | **37%** |

### 720p 升级档（add-on / 加价方案候选）

| 用户档位 | 视频条数/月 | Seedance 成本 | 总成本 | 毛利 (¥1000) | 毛利 (¥1500) |
|---|---|---|---|---|---|
| 重度 | 60 | ¥1140 | ¥1230 | **-23% 🔴** | 18% |

**结论**：
- **480p / 60条** = 37% 毛利，可接受（接近 §3 目标 50%）— **MVP-1 默认**
- **720p / 60条** 在 ¥1000 ARPU 下亏本；可作为 ARPU ¥1500+ 升级档卖
- **数据源摊销 ¥30/用户/月**仍是 D31 占位，待新榜合同月费回填

**Spend cap 现状**：`monthly_usage.video_count >= 60` + `total_cost_fen >= 50_000` 双闸已上线（W1-07）。Token 计量已直接挂 `usage.completion_tokens`（D32），不再依赖 D24 的"按秒估"。
