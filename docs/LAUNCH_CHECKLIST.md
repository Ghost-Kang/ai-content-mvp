...
> **最后更新**：2026-05-08（happy path 红线重定 + 内测首日 incident 复盘）
...
|---|---|---|---|---|
| 合规 | 4 | 4 | 0 | 0 |
| 功能 | 4 | 3 | 1（happy path 红线重定，连验 5 日） | 0 |
| 运营 | 4 | 4（PostHog 降级签字 / 3 seed 已发） | 0 | 0 |
| 可维护 | 3 | 3 | 0 | 0 |
| **合计** | **15** | **14** | **1** | **0** |

**当前结论**：🟡 **NEEDS WORK** · 1 项待 5 日聚合数据收尾（happy path P50/P95，新红线 7min/10min）
...
### [~] Happy path E2E：Signup → Quick Create → Script → Review → Export — **目标重定（2026-05-08）**

- **状态**：⚠️ 目标重定：原「< 5 分钟」红线撤回，新红线 **P50 < 7 min · P95 < 10 min**（auto step 时间，不含 Solo Review 用户思考时间）
- **重定理由**（基于 2026-05-08 内测真实数据，2 个 done run + 5 节点 step 耗时实测）：

  | 节点 | P50 | P95 | 占 5min 预算 |
  |---|---|---|---|
  | topic | 1s | 2s | 0.3% / 0.7% |
  | script | 92s | 205s | 31% / **68%** |
  | storyboard | 33s | 33s | 11% |
  | video | 165s | 272s | 55% / **91%** |
  | export | 25s | 26s | 8% |
  | **合计** | **316s = 5.3 min** | **538s = 9 min** | **105% / 179%** |

  P50 实测已经爆 5 min 红线 5%，P95 爆 79%。原目标在 video 节点结构性 50-90s/帧 × 17 帧的 Seedance 延迟下从未 budget-feasible。

- **本次落地的 P95 优化**（已 commit，待部署）：
  - `script.ts`：`MAX_LLM_RETRIES: 3 → 2`（commit `711d467`）—— P95 -60s
  - `LLM_PROVIDER_TIMEOUT_MS=90000` env（待 Vercel set）—— 减少 kimi 60s 抽风时假性失败
  - 部署后预计 P95 降到 ~480s（8 min），仍超原 5 min 但符合新 10 min 红线
- **怎么验（新流程）**：用 `pnpm prod:today` 抽 `step_index, dur_sec` 的 P50 / P95，连续 5 个工作日各抽一次取均值。验收门槛：5 天均值都 ≤ 新红线即通过。
- **不再**：录单次屏 + 秒表（单次样本噪声大）—— 改用聚合统计。
- **后置监控**：连续 3 天 P95 > 10 min 重启 script step 重写或 video provider 评估。
...
- Happy path 5 日 P95 中位 > 12 min（重定红线 10 min 的 +20% 容忍带）
- 内测期间出现「run 卡死 > 1 小时无人工干预释放」（watchdog 失效信号）

**可以降级**的情况：
- 字数合规 ≥ 45%（已决定）
- 抑制词捕获率 ≥ 95% 但非 100%
- 种子用户只招到 2 人（硬推不推到 3）
- Happy path P95 在 10-12 min 之间（接近重定红线但不超 +20%）
