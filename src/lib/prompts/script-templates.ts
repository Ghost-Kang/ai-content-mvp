...
总字数目标 205 字 = 17 帧 × 平均 12 字。**任何一帧超过 14 字 = 该帧应该拆成两帧；任何一帧不足 8 字 = 该帧应该合并。**

| 区段 | 帧号 | 帧数 | 每帧时长 | 每帧字数 | 累计字数 |
|---|---|---|---|---|---|
| 钩子（反常识） | 1-3 | 3 | 2s | 12-14 | ~40 |
| 痛点/论点展开 | 4-7 | 4 | 3s | 13-14 | ~55 |
| 案例证据 | 8-13 | 6 | 3-4s | 12-14 | ~80 |
| 真相揭示 + 金句 | 14-17 | 4 | 3-5s | 8-10 | ~35 |
| **合计** | | **17** | **60s** | **均 12** | **200-210** |

每一帧 = 一个画面切换 = 一个小意群。不要把两句话塞进同一帧。如果一帧超过 14 字，大概率应该拆成两帧。
...
2. 所有 text 拼起来去空白后在 **200-210** 字之间吗？**这是硬线，不在窗口内必须改。**
3. 开场 3 帧每帧是 2 秒快切吗？
4. 没有任何一帧超过 14 字吗？（超过说明你应该拆）
5. 没有任何一帧不足 8 字吗？（不足说明你应该合并或扩写）

**如果自检不过：**
- 字数超了 → 找最啰嗦的 2-3 帧，每帧删 2-3 个字
- 字数不足 → 找案例证据段（帧 8-13）最薄的帧，每帧补 2-3 个具体细节字
- 不要通过改帧数来凑字数
