...
**指标 2 — 所有 frame.text 拼接后去空白字符数必须在 200-215 之间**（理想 205-210）

在你开始写之前，先在心里数一遍：你打算写几帧？如果不是 16/17/18，立即重新规划。
在你输出之前，再数一遍 frames.length 和总字数。这两个是我的验证脚本检查的前两件事。

## 17 帧的标准分配（照这个模板拆）

总字数目标 **205-210 字** = 17 帧 × 平均 12-13 字。字数不足比超出更糟 —— 不足意味着内容稀薄，超出只是稍微赶节奏。**偏向 208 而不是 200。**

| 区段 | 帧号 | 帧数 | 每帧时长 | 每帧字数 | 累计字数 |
|---|---|---|---|---|---|
| 钩子（反常识/现象） | 1-3 | 3 | 2s | 13-14 | ~40 |
| 痛点/论点展开 | 4-7 | 4 | 3s | 13-14 | ~55 |
| 案例证据 | 8-13 | 6 | 3-4s | 13-14 | ~82 |
| 真相揭示 + 金句 | 14-17 | 4 | 3-5s | 8-10 | ~36 |
| **合计** | | **17** | **60s** | **均 12.5** | **~213** |

每一帧 = 一个画面切换 = 一个小意群。不要把两句话塞进同一帧。每帧字数硬线 **8-15**（超过 15 必须拆，不足 8 必须合并）。
...
2. 所有 text 拼起来去空白后在 **200-215** 字之间吗？**硬线。**
3. 开场 3 帧每帧是 2 秒快切吗？
4. 没有任何一帧超过 15 字吗？（超过必须拆）
5. 没有任何一帧不足 8 字吗？（不足必须合并或扩写）

**字数自救指南（必须执行到通过为止）：**

- **字数 < 200（严重不足）** → 问题几乎总是案例段（帧 8-13）太抽象。给每帧补一个**具体数字/人名/时间/品牌名**（例："客户"→"32 人团队的 SaaS 客户"；"几个月"→"连续 3 个月 KPI 垫底"）。不要说废话凑字，加的必须是信息。
- **字数 200-215（合规）** → 直接输出。
- **字数 > 215（稍超）** → 找钩子或金句段里最啰嗦的 1-2 帧，每帧删 2-3 个字。不要动案例证据段。
- **字数 > 225（严重超）** → 整体太满，每个区段各删 2 字。

**不要通过增减帧数来凑字数。17 帧是锁死的。**
